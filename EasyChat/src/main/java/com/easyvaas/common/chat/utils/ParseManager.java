...
public class ParseManager {
        ...
        try {
            ...
            if (e.getCode() == ParseException.OBJECT_NOT_FOUND) {
                ...
                try {
                    ...
                    e1.printStackTrace();
                    ChatLogger.e(TAG, "parse error " + e1.getMessage());
                }

            }
            e.printStackTrace();
            ChatLogger.e(TAG, "parse error " + e.getMessage());
        }
        return false;
    }

    public void getContactInfos(List<String> usernames, final EMValueCallBack<List<ChatUser>> callback) {
        ParseQuery<ParseObject> pQuery = ParseQuery.getQuery(CONFIG_TABLE_NAME);
     ...
     */
    private static void setUserHeader(ChatUser user) {
    ...
    public void asyncGetCurrentUserInfo(final EMValueCallBack<ChatUser> callback) {
                ...
                if (error == ParseException.OBJECT_NOT_FOUND) {
                    ...
                    pUser.saveInBackground(new SaveCallback() {
                        ...
                        public void done(ParseException arg0) {
                            if (arg0 == null) {
                                callback.onSuccess(new ChatUser(username));
                            }
                        }
                    });
                } else {
        ...
        pQuery.getFirstInBackground(new GetCallback<ParseObject>() {
            ...
            public void done(ParseObject pUser, ParseException e) {
                if (pUser != null) {
                    ...
                    if (callback != null) {
                        ChatUser user = ChatUserUtil.getUserInfo(username);
                        if (user != null) {
                            user.setNick(nick);
