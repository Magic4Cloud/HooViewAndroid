...
public class UserVLivingAdapter extends MyBaseAdapter {
     ...
     * 设置图文直播数据
     */
    public void setHeaderModel(CheckImageTextLiveModel imageTextModel) {
        if (mHasHeader) {
    ...
    class ImageTextViewHolder extends BaseViewHolder {
        ...
        void setModel(final CheckImageTextLiveModel model) {
            if (model != null) {
                // follow
                DecimalFormat df2 = new DecimalFormat("###,###");
                mFollowTv.setText(mContext.getString(R.string.image_text_follow_count, df2.format(model.getData().getViewcount())));
                // name
                mNameTv.setText(model.getData().getName());
                // tag
                List<UserInfoModel.TagsEntity> tags = model.getData().getTags();
                ...
                mRootView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        if (Preferences.getInstance(mContext).isLogin() && EVApplication.isLogin() && EVApplication.getUser() != null && model.getData().getOwnerid().equals(EVApplication.getUser().getName()))
                            MyImageTextLiveRoomActivity.start(getContext(), model);
                        else {
    ...
    class VideoViewHolder extends BaseViewHolder {
        ...
        void setModel(final VideoEntity videoEntity) {
            if (videoEntity != null) {
                ...
                // watch_count
                mWatchCountTv.setText(mContext.getString(R.string.watch_count, videoEntity.getWatch_count() + ""));
                // title
