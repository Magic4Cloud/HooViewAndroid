...
import com.easyvaas.elapp.bean.SearchInfoEntity;
...
public class ApiHelper {
    ...
    public void getUserInfosNew(List<String> names, MyRequestCallBack<UserInfoArrayModel> callBack) {
        Map<String, String> map = new HashMap<>();
        if (names == null || names.size() == 0) {
            return;
        }
        ...
        map.put("sessionid", mPref.getSessionId());
        map.put("namelist", nameList);
        sRequestHelper.getAsGson(ApiConstant.USER_INFOS, map, UserInfoArrayModel.class, callBack);
    }
    ...
    public void userEditInfo(String nickname, String birthday, String location,
        ...
        if (TextUtils.isEmpty(location)) {
            map.put("location", mContext.getString(R.string.default_user_location));
        } else {
            map.put("location", location);
        }
    ...
    public void userEditInfo(String nickname, String birthday, String location,
                             String gender, String signature,String credentials, MyRequestCallBack<String> callBack) {
        Map<String, String> map = new HashMap<>();
        map.put("sessionid", mPref.getSessionId());
        map.put("nickname", nickname);
        map.put("birthday", birthday);
    ...
    public void videoCallRequest(String ownerName, MyRequestCallBack<VideoCallEntity> callBack) {
        Map<String, String> param = new HashMap<>();
        param.put("sessionid", mPref.getSessionId());
        param.put("ownername", ownerName);
        sRequestHelper.getAsGson(ApiConstant.VIDEO_CALL_REQUEST, param, VideoCallEntity.class, callBack);
    }

    public void videoCallAccept(String requestName, MyRequestCallBack<VideoCallEntity> callBack) {
        Map<String, String> param = new HashMap<>();
        param.put("sessionid", mPref.getSessionId());
        param.put("requestname", requestName);
        sRequestHelper.getAsGson(ApiConstant.VIDEO_CALL_ACCEPT, param, VideoCallEntity.class, callBack);
    }

    public void videoCallEnd(String callId, String vid, MyRequestCallBack<String> callBack) {
        Map<String, String> param = new HashMap<>();
        param.put("sessionid", mPref.getSessionId());
        param.put("callid", callId);
        param.put("vid", vid);
        sRequestHelper.getAsString(ApiConstant.VIDEO_CALL_END, param, callBack);
    }

    public void videoCallCancel(String callId, String vid, MyRequestCallBack<String> callBack) {
        Map<String, String> param = new HashMap<>();
        param.put("sessionid", mPref.getSessionId());
        param.put("callid", callId);
        param.put("vid", vid);
        sRequestHelper.getAsString(ApiConstant.VIDEO_CALL_CANCEL, param, callBack);
    }

    public void getCollectList(String type, MyRequestCallBack<CollectListModel> callBack) {
        Map<String, String> param = new HashMap<>();
        param.put("sessionid", mPref.getSessionId());
        param.put("type", type);
        sRequestHelper.getAsGson(ApiConstant.COLLECTLIST, param,CollectListModel.class,callBack);
    }

    /**
     * 名称	类型	说明	取值范围	必选
     * sessionid	string	会话id	-	是
     * type	int	类型	1-资讯,2-视频,3-指数,4-股票	是
     * action	int	操作	1-添加,2-删除	是
     * code	int	收藏唯一标识	-	是
     */
    public void collect(String code, String type, String action, MyRequestCallBack<String> callBack) {
        Map<String, String> param = new HashMap<>();
        param.put("sessionid", mPref.getSessionId());
        param.put("code", code);
        param.put("type", type);
        param.put("ation", action);
        sRequestHelper.getAsString(ApiConstant.COLLECT, param, callBack);
    }

    /**
     * sessionid	string	会话id	-	是
     *
     * @param type     int	类型	1-资讯,2-视频,3-指数,4-股票
     * @param callBack
     */
    public void getUserHistory(String type, MyRequestCallBack<HistoryListModel> callBack) {
        Map<String, String> param = new HashMap<>();
        param.put("sessionid", mPref.getSessionId());
        param.put("type", type);
