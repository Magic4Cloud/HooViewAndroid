...
public class TextActivity extends BaseActivity {
    ...
    private String mAgreementText =
            ...
            "本《协议》由易视云随时更新，更新后的协议条款一旦公布即代替原来的协议条款, 恕不再另行通知, 用户可在应用上查阅最新版协议条款。在修改《协议》条款后, 如果用户不接受修改后的条款, 请立即停止使用本应用服务, 继续使用的用户将被视为已接受了修改后的协议。 \n" +
            "二、服务内容 " +
            "\n" +
            "1、易视云是移动、直播、分享、交互的全新社交平台，具有互动、分享、社交等功能。 " +
            "\n" +
            ...
            "三、用户说明 \n" +
            "1、完成注册程序的用户均为易视云用户。\n" +
            "2、用户有义务保证密码和账号的安全, 用户利用该账号所进行的一切活动引起的任何损失或损害, 由用户自行承担全部责任。如用户发现账号遭到未授权的使用或发生其他任何安全问题, 应立即修改账号密码并妥善保管。因黑客行为或用户的保管疏忽导致账号非法使用, 易视云不承担任何责任。\n" +
            "3、用户账号在丢失、遗忘密码后, 须遵照本应用程序的密码找回程序找回账号密码。对用户因被他人冒名申请而致的任何损失, 易视云不承担任何责任。 \n" +
            ...
            "5、用户将其在易视云上发表的全部内容，授予易视云免费的、非独家使用许可，易视云有权将该内容用于易视云各种形态的产品和服务上，包括但不限于网站以及发表的应用或其他互联网产品。第三方若出于非商业目的，将用户在易视云上发表的内容转载在易视云之外的地方，应当注明”转自易视云”，并不得对内容进行修改演绎。若需要进行修改，或用于商业目的，第三方应当联系用户获得单独授权，按照用户规定的方式使用该内容。\n" +
            "6、任何用户发现易视云上有内容涉嫌侮辱或者诽谤他人、侵害他人合法权益的或违反本协议的，均有权直接举报。\n" +
            "7、为了给广大用户提供一个优质社交平台，确保良性健康发展，易视云将对涉及反动、色情和发布不良内容的用户，进行严厉处理。一经发现，将给予永久封禁账号并清空所有内容的处理。\n" +
            "四、隐私保护 \n" +
            "易视云不对外公开或向第三方提供用户的注册资料及及用户在使用网络服务时存储在本平台的非公开内容，但下列情况除外: \n" +
            "1、事先获得用户的明确授权;\n" +
            "2、根据有关的法律法规要求;\n" +
            "3、按照相关政府主管部门的要求;\n" +
            "4、第三方同意承担与易视云同等的保护用户隐私的责任；在不透露单个用户隐私资料的前提下，易视云有权对整个用户数据库进行分析并对用户数据库进行商业上的利用。\n" +
            "五、免责申明\n" +
            "1、平台上发布的内容仅代表用户观点，与易视云无关。作为内容的发表者，需自行对所发表内容负责，因所发表内容引发的一切纠纷，由该内容的发表者承担全部法律及连带责任。易视云不承担任何法律及连带责任。\n" +
            "2、易视云不保证网络服务一定能满足用户的要求，也不保证网络服务不会中断，对网络服务的及时性、安全性、准确性也都不作保证。\n" +
            "3、易视云如因系统维护或升级等而需暂停服务时，将事先公告。对于因不可抗力或易视云不能控制的原因造成的网络服务中断或信息丢失等其它问题，易视云不承担任何责任，但将尽力减少因此而给用户造成的损失和影响。\n" +
            "\n" +
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_privacy_service);
        LinearLayout linearLayout = new LinearLayout(this);
        linearLayout.setLayoutParams(new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        TextView textView = (TextView) findViewById(R.id.content_txv);

        int type = getIntent().getIntExtra(EXTRA_TYPE, TYPE_AGREEMENT);
        if(type == TYPE_PHONE_BIND_ISSUE) {
            textView.setText(mPhoneAappeal);
        }else if (type == TYPE_AGREEMENT) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.findItem(R.id.menu_complete).setTitle(R.string.to_appeal);
        ...
        switch (item.getItemId()) {
            case R.id.menu_complete:
                ...
                try {
                    ...
                    Toast.makeText(TextActivity.this, getString(R.string.no_email_applications_installed),
