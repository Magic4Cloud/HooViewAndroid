...
public class UserFansActivity extends MyBaseActivity {
    ...
    protected String getTitleText() {
        if (EVApplication.isLogin()) {
            if (userId.equals(EVApplication.getUser().getName()))
                return getString(R.string.user_myfans);
        }
        return getString(R.string.user_tafans);
    }
    ...
    protected void initViewAndData() {
        ...
        manager.beginTransaction().add(R.id.user_collection_layout, UserFansFragment.newInstance(userId,sessionId)).commit();
    }
}
