...
public abstract class BaseRvcFragment extends AbstractListFragment {
    ...
    public void initListView() {
        mTapTopView = getView() == null ? null : getView().findViewById(R.id.tap_top_iv);
    ...
    protected void onRefreshComplete(int updateCount) {
        ...
        if (isListViewEmpty()) {
            showEmptyView(EmptyView.TYPE_EMPTY, getString(R.string.empty_title));
        } else {
            hideEmptyView();
            if (updateCount == 0) {
    ...
    protected void onRequestFailed(String msg) {
        ...
        if (isListViewEmpty()) {
            showEmptyView(EmptyView.TYPE_ERROR, getString(R.string.msg_network_bad_check_click_retry));
        }
    }
