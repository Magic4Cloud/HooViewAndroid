package com.easyvaas.common.chat.applib.controller;
...
public abstract class HXSDKHelper {
    ...
    private static ArrayList<EMCallBack> callBacks = new ArrayList<>();
    ...
    public synchronized boolean onInit(Context context) {
        ...
        EMChat.getInstance().setDebugMode(hxModel.isDebugMode());

        ChatLogger.d(TAG, "initialize EMChat SDK");
    ...
    protected void initHXOptions() {
        ChatLogger.d(TAG, "init HuanXin Options");
    ...
    protected void initListener() {
        ChatLogger.d(TAG, "init listener");
    ...
    /**
     * check the application process name if process name is not qualified,
     * then we think it is a service process and we will not init SDK
     */
        ...
        while (i.hasNext()) {
            ...
            try {
                if (info.pid == pID) {
                    ...
                    // ChatLogger.d("Process", "Id: "+ info.pid +" ProcessName: "+
                ...
                // ChatLogger.d("Process", "Error>> :"+ e.toString());
            }
        }
        return processName;
    }
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    for (EMCallBack emCallBack : callBacks) {
    ...
    public List<EMGroup> getLocalAllGroupList() {
        return EMGroupManager.getInstance().getAllGroups();
    }

    public EMConversation getGroupConversation(String groupId) {
        return EMChatManager.getInstance()
                .getConversationByType(groupId, EMConversation.EMConversationType.GroupChat);
    }

    public void createPublicGroup(
            final String groupName, final String desc, final String[] members, final boolean allowInvite,
            final int maxUsers, final EMValueCallBack callBack) {
        new Thread() {
            @Override
            public void run() {
                try {
                    EMGroup emGroup = EMGroupManager.getInstance()
                            .createPrivateGroup(groupName, desc, members, allowInvite, maxUsers);
    ...
    public void getGroupInfo(final String groupId, final EMValueCallBack callBack) {
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public EMGroup getLocalGroupInfo(String groupId) {
        return EMGroupManager.getInstance().getGroup(groupId);
    }

    public void changeGroupName(final String groupId, final String changedGroupName,
    ...
    public void dissolutionGroup(final String groupId, final EMValueCallBack callBack) {
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public void quitGroup(final String groupId, final EMValueCallBack callBack) {
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public void blockGroupMessage(final String groupId, final EMCallBack callBack) {
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public void unblockGroupMessage(final String groupId, final EMCallBack callBack) {
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public void addUsersToGroup(final String groupId, final String[] newMembers, final EMCallBack callBack) {
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public void inviteUser(final String groupId, final String[] newMembers, final EMCallBack callBack) {
        new Thread() {
            @Override
            public void run() {
                try {
    ...
    public void removeUsersFromGroup(final String groupId, final String[] removeMembers,
