...
public class UserProfileActivity extends BaseActivity implements OnClickListener {
    ...
    private void initListener() {
        ...
        if (username == null) {
            ...
            tvNickName.setText(ChatUserUtil.getCurrentUserNickname());
            ChatUserUtil.setCurrentUserAvatar(this, headAvatar);
            ...
            tvNickName.setText(ChatUserUtil.getNickName(username, this));
            ChatUserUtil.setUserAvatar(this, username, headAvatar);
    ...
    public void asyncFetchUserInfo(String username) {
        ((ChatHXSDKHelper) HXSDKHelper.getInstance()).getUserProfileManager()
                .asyncGetUserInfo(username, new EMValueCallBack<ChatUser>() {
                    @Override
                    public void onSuccess(ChatUser user) {
                        if (user != null) {
                            tvNickName.setText(user.getNick());
                            if (!TextUtils.isEmpty(user.getAvatar())) {
                                CommonUtils.showImage(user.getAvatar(), R.drawable.somebody, headAvatar);
                            } else {
                                headAvatar.setImageResource(R.drawable.somebody);
                            }
                            ChatHXSDKHelper.getInstance().saveContact(user);
                        }
                    }
    ...
    private void updateRemoteNick(final String nickName) {
        dialog = ProgressDialog
                .show(this, getString(R.string.dl_update_nick), getString(R.string.dl_waiting));
        new Thread(new Runnable() {

            @Override
            public void run() {
                boolean updatenick = ((ChatHXSDKHelper) HXSDKHelper.getInstance()).getUserProfileManager()
                        .updateParseNickName(nickName);
                ...
                if (!updatenick) {
                    runOnUiThread(new Runnable() {
                        public void run() {
                            Toast.makeText(UserProfileActivity.this,
                                    getString(R.string.toast_update_nickname_fail), Toast.LENGTH_SHORT)
                                    .show();
                            dialog.dismiss();
                        }
                    });
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            dialog.dismiss();
                            Toast.makeText(UserProfileActivity.this,
                                    getString(R.string.toast_update_nickname_success), Toast.LENGTH_SHORT)
                                    .show();
    ...
    private void uploadUserAvatar(final byte[] data) {
        dialog = ProgressDialog
                .show(this, getString(R.string.dl_update_photo), getString(R.string.dl_waiting));
        new Thread(new Runnable() {

            @Override
            public void run() {
                final String avatarUrl = ((ChatHXSDKHelper) HXSDKHelper.getInstance()).getUserProfileManager()
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (avatarUrl != null) {
                            ...
                            Toast.makeText(UserProfileActivity.this,
                                    getString(R.string.toast_update_photo_fail),
                                    Toast.LENGTH_SHORT).show();
                        }
