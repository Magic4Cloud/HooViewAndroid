package com.easyvaas.common.emoji.db;
...
public class DBHelper {
    ...
    public EmoticonBean queryEmoticonBean(String contentStr) {
        String sql =
                "select * from " + TABLE_NAME_EMOTICONS + " where " + TableColumns.EmoticonColumns.CONTENT
                        + " = '" + contentStr + "'";
        Cursor cursor = db.rawQuery(sql, null);
        try {
            if (cursor.getCount() > 0) {
                cursor.moveToFirst();
                long eventType = cursor
                        .getLong(cursor.getColumnIndex(TableColumns.EmoticonColumns.EVENTTYPE));
                String content = cursor
                        .getString(cursor.getColumnIndex(TableColumns.EmoticonColumns.CONTENT));
                String iconUri = cursor
                        .getString(cursor.getColumnIndex(TableColumns.EmoticonColumns.ICONURI));
    ...
    public ArrayList<EmoticonSetBean> queryEmoticonSet(String sql) {
        ...
        try {
            ...
            if (count > 0) {
                ...
                for (int i = 0; i < count; i++) {
                    String name = cursor
                            .getString(cursor.getColumnIndex(TableColumns.EmoticonSetColumns.NAME));
                    ...
                    String iconUri = cursor
                            .getString(cursor.getColumnIndex(TableColumns.EmoticonSetColumns.ICONURI));
                    String iconName = cursor
                            .getString(cursor.getColumnIndex(TableColumns.EmoticonSetColumns.ICONNAME));
                    boolean isshowdelbtn =
                            cursor.getInt(cursor.getColumnIndex(TableColumns.EmoticonSetColumns.ISSHOWDELBTN))
                                    == 1;
                    int itempadding = cursor
                            .getInt(cursor.getColumnIndex(TableColumns.EmoticonSetColumns.ITEMPADDING));
                    int horizontalspacing = cursor
                            .getInt(cursor.getColumnIndex(TableColumns.EmoticonSetColumns.HORIZONTALSPACING));
                    int verticalSpacing = cursor
                            .getInt(cursor.getColumnIndex(TableColumns.EmoticonSetColumns.VERTICALSPACING));

                    ArrayList<EmoticonBean> emoticonList = null;
                    if (!TextUtils.isEmpty(name)) {
                        String sqlGetEmoticonBean = "select * from " + TABLE_NAME_EMOTICONS + " where "
                                + TableColumns.EmoticonColumns.EMOTICONSET_NAME + " = '" + name + "'";
                    ...
                    EmoticonSetBean bean = new EmoticonSetBean(name, line, row, iconUri, iconName,
                            isshowdelbtn, itempadding, horizontalspacing, verticalSpacing, emoticonList);
                    beanList.add(bean);
                    cursor.moveToNext();
                }
                return beanList;
